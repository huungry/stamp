...
class RewardRouter(
  ...
  private val listRewardEndpoint: ServerEndpoint[Any, IO] =
    bearerEndpoint.get
      ...
      .errorOutVariants(
        oneOfVariant(statusCode(StatusCode.NotFound).and(jsonBody[ListRewardError.RestaurantUserNotFound])),
        oneOfVariant(statusCode(StatusCode.Forbidden).and(jsonBody[ListRewardError.NotManager])),
        oneOfDefaultVariant(statusCode(StatusCode.BadRequest).and(jsonBody[ListRewardError]))
      )
      .serverLogic(userId => restaurantId => listRewardService.listActive(userId, restaurantId))
